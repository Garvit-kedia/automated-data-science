...
from sklearn.ensemble import RandomForestClassifier,RandomForestRegressor,AdaBoostClassifier,AdaBoostRegressor,GradientBoostingClassifier,GradientBoostingRegressor,StackingClassifier,StackingRegressor
...
all_type=["select your type","regression","classification","deep learning"]
...
if(a=="classification"):
    ...
    #                       
        ...
        if st.sidebar.checkbox("feature scaling"):
            try:
                        all_columns = df.columns.to_list()
                        target= st.sidebar.selectbox("Select Target Column", all_columns,key="gopi")
        ...
        if st.sidebar.checkbox("ensemble"):
            ...
            selct_models = st.multiselect("Select models for ", options=all_models, format_func=lambda x: type(x).__name__)
    ...
    # Train the models
        ...
        if st.sidebar.checkbox('Stacking '):
            selected_base_models = st.multiselect("Select base models to be stacked", list(all_models.keys()))

                        # User selection for final model to use after stacking
            final_model = st.selectbox("Select the final model to be used", list(all_models.keys()))

            try:
                            base_models = [(model_name, all_models[model_name]) for model_name in selected_base_models]
        ...
        if st.sidebar.button("Run model"):
            ...
            if model_name == 'Gradient Boosting Classifer':
                            try:
                                ...
                                probabilities = model.predict_proba(X_test)[:,1] 
                                Gradientc=ClassificationMetrics(y_test,predictions,probabilities)
        ...
        if st.sidebar.checkbox("optimisation"):
            if model_name=='Logistic Regression':
                        # try:
                                ...
                                model=RandomizedSearchCV(LogisticRegression(),param_distributions=parameter,cv=5)
                                ...
                                if st.sidebar.button("run optimised"):
                                    ...
                                    predictions=model.predict(X_test)
                                    probabilities = model.predict_proba(X_test)[:,1] 
                                    logitichc=ClassificationMetrics(y_test,predictions,probabilities)
            ...
            if model_name=='Decison Tree Classifer':
                        # try:        
                            ...
                            model=RandomizedSearchCV(DecisionTreeClassifier(),param_distributions=parameter,cv=5)
                        ...
                        #         st.write(" ")
                            if st.sidebar.button("run optimised"):
                                ...
                                decisonhc=ClassificationMetrics(y_test,predictions,probabilities)
            ...
            if model_name == 'KNeighborClassifier':
                            ...
                            model=RandomizedSearchCV(KNeighborsClassifier(),param_distributions=parameter,cv=5,verbose=3)
            ...
            if model_name == 'Gradient Boosting Classifer':
                            ...
                            model=RandomizedSearchCV(GradientBoostingClassifier(),param_distributions=parameter,cv=5,verbose=3)
            ...
            if model_name == 'Random Forest Classifier':
                            ...
                            model=RandomizedSearchCV(RandomForestClassifier(),param_distributions=parameter,cv=5,verbose=3)
            ...
            if model_name == 'Adaboost Classifer':
                            ...
                            tmodel=RandomizedSearchCV(AdaBoostClassifier(),param_distributions=parameter,cv=5,verbose=3)
                            # st.write(model.best_params_)
        ...
        if st.sidebar.checkbox("optimisation"):
            ...
            if model_name=='Decison Tree Regressor':
                        ...
                        try:  
                            ...
                            model=RandomizedSearchCV(DecisionTreeRegressor(),param_distributions=parameter,cv=5,n_jobs=-1)
            ...
            if model_name == 'KNeighborRegressor':
                        try:
                            ...
                            model=RandomizedSearchCV(KNeighborsRegressor(),param_distributions=parameter,cv=5,n_jobs=-1)
            ...
            if model_name == 'Gradient Boosting Regressor':
                        try:
                            ...
                            model=RandomizedSearchCV(GradientBoostingRegressor(),param_distributions=parameter,cv=5,n_jobs=-1)
            ...
            if model_name == 'Random Forest Regressor':
                        try:
                            ...
                            model=RandomizedSearchCV(RandomForestRegressor(),param_distributions=parameter,cv=5,n_jobs=-1)
            ...
            if model_name == 'Adaboost Regressor':
                        try:
                            ...
                            model=RandomizedSearchCV(AdaBoostRegressor(),param_distributions=parameter,cv=5,n_jobs=-1)
            ...
            if model_name=='Support Vector Regressor':
                        try:
                            ...
                            model=RandomizedSearchCV(SVR(),param_distributions=parameter,cv=5,n_jobs=-1)
                        except:
                               st.write()
